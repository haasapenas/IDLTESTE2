...
import { jogosService } from '../lib/jogosService'
...
export const useJogos = () => {
  ...
  // Carregar jogos na inicialização e quando o usuário mudar
  useEffect(() => {
    // Só carregar jogos se a autenticação não estiver carregando
    if (!authLoading) {
      carregarJogos()
    }
  }, [carregarJogos, user, authLoading])
